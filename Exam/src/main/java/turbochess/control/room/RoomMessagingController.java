...
public class RoomMessagingController extends RoomController{
    ...
    @SendTo("/queue/{room}")
    @Transactional
    public ClientPacket sendMove(@DestinationVariable String room, @Payload MovePacket clientPacket, Principal principal) {
        ...
        try{
            User user = getUserByUsername(usernameFrom);
            Room contextRoom = roomService.getRoomByCode(room);
